...
export const getInsights = async (req, res) => {
    try {
        ...
        const rows = result.rows.map(row => ({
            ...row,
            insight_text: JSON.parse(row.insight_text),
        }));
...
const buildMonthlyInsight = async (userId) => {
    ...
    const totalIncome = parseFloat(row.income ?? 0) || 0;
    const totalExpenses = parseFloat(row.expense ?? 0) || 0;
    ...
    const content = await generateMonthlyInsight({
        ...
        previousMonths: (row.trend || []).map(t => ({
            ...
            expense: parseFloat(t.expenses) || 0,
        })),
        currency,
    });
    ...
    return { content, insightType: 'monthly_summary', periodStart, periodEnd };
};
...
const buildSavingsTips = async (userId) => {
    ...
    const content = await generateSavingsTips({
        ...
        monthlyIncome: parseFloat(incomeResult.rows[0]?.income ?? 0) || 0,
        currency,
    });

    return { content, insightType: 'savings_tips' };
};
...
const buildBudgetAlert = async (userId, categoryId) => {
    ...
    return { content, insightType: 'budget_alert' };
};
...
export const generateInsight = async (req, res) => {
    ...
    try {
        ...
        const insightText = JSON.stringify(result.content);

        const inserted = await pool.query(
            `INSERT INTO ai_insights (user_id, insight_text, type, period_start, period_end)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *`,
            [req.userId, insightText, result.insightType, result.periodStart ?? null, result.periodEnd ?? null]
        );
