...
// ------------------------------------------------------------------
// generateTransactions – creates 12 months of fake transactions
// ------------------------------------------------------------------
const generateTransactions = (catMap) => {
  ...
  for (let monthsAgo = 11; monthsAgo >= 0; monthsAgo--) {
    ...
    if (rng() < 0.5) {
      add(rangeInt(8, 25), 'Other Income', rangeFloat(15, 120), 'income', pick(['Cashback', 'Refund', '']));
    }
    ...
    for (let day = 1; day <= monthLastDay; day++) {
      const d = new Date(monthStart.getFullYear(), monthStart.getMonth(), day);
      if (d.getDay() === 0) {
        add(day, 'Groceries', rangeFloat(55, 120), 'expense', pick(['Whole Foods', "Trader Joe's", 'Week 1']));
      }
    }
...
// ------------------------------------------------------------------
// seed – main function to populate the database with demo data
// ------------------------------------------------------------------
const seed = async () => {
  ...
  try {
    ...
    // ---- 1. Remove existing demo user (if any) ----
    ...
    for (const b of BUDGETS) {
      const catId = catMap[b.name];
      if (!catId) {
        console.warn(`⚠️  Category "${b.name}" not found – skipping budget.`);
        continue;
      }
      await client.query(
