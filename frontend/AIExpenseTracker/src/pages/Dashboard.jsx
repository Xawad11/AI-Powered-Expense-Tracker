...
const Dashboard = () => {
    ...
    useEffect(() => {
        const load = async () => {
            const [s, t, b, r, bd] = await Promise.allSettled([
            ...
            else console.error('Summary failed:', s.reason);

            if (t.status === 'fulfilled') setTrend(t.value.data);
            else console.error('Trend failed:', t.reason);

            if (b.status === 'fulfilled') setBreakdown(b.value.data);
            else console.error('Breakdown failed:', b.reason);

            if (r.status === 'fulfilled') setRecent(r.value.data);
            else console.error('Recent failed:', r.reason);

            if (bd.status === 'fulfilled') setBudgets(bd.value.data);
            else console.error('Budgets failed:', bd.reason);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <KpiCard
                    label="Income"
                    value={formatCurrency(summary?.incomeThisMonth ?? 0, currency)}
                ...
                <KpiCard
                    label="Expenses"
                    value={formatCurrency(summary?.expenseThisMonth ?? 0, currency)}
