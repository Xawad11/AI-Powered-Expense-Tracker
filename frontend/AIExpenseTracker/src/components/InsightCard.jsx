...
const previewText = (insight) => {
    const c = insight.insight_text;
    if (insight.type === 'monthly_summary') return c?.summary || '';
    if (insight.type === 'budget_alert') return c?.message || c?.title || '';
    if (insight.type === 'savings_tips') return c?.overallTip || '';
    return '';
};

const headerChip = (insight) => {
    const c = insight.insight_text;
    if (insight.type === 'monthly_summary' && typeof c?.healthScore === 'number') {
    ...
    if (insight.type === 'budget_alert' && c?.severity) {
...
const InsightCard = ({ insight, defaultExpanded = false }) => {
    ...
    const c = insight.insight_text;
    const t = typeStyles[insight.type] || typeStyles.monthly_summary;
        ...
        <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden hover:border-slate-200 transition">
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                        <h3 className="font-bold text-slate-900">{labelMap[insight.type]}</h3>
                        {headerChip(insight)}
            ...
            {expanded && (
                <div className="px-5 pb-6 border-t border-slate-100 pt-5">
                    {insight.type === 'monthly_summary' && <MonthlySummaryView c={c} />}
                    {insight.type === 'budget_alert' && <BudgetAlertView c={c} />}
                    {insight.type === 'savings_tips' && <SavingsTipsView c={c} />}
                </div>
            )}
        </div>
    );
